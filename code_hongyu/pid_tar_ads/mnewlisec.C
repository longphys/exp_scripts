...
I=&J;
char str3He[3][20]={"1H-CD2.txt","1H-Al.txt","1H-Si.txt"};
for(Int_t _thick=300;_thick<=300;_thick++){
if(_thick%10==0)cout<<"thick="<<_thick<<'\n';
AA=1;ZZ=1; openandreadandtest(se,AA,str3He,_thick); getit(f,se,AA,ZZ,I,_thick); empty(se);
}
...
if(in2.is_open()){//检查文件是否存在
 ...
 Int_t key=0;
 double E0=0,E1=0,Esmall=0,E2=0,E=0,dE=0,e1=0,e2=0,e3=0,x=0,dx=0.025;//E0为能量总值，E为能量实时记录值，E1为穿透第一块硅时的能量，E2为穿透第二块时的能量
 Double_t d[3]={300,500,100000.};
 ...
 if(i<=NUM/2)E0=E=(Double_t)(i)*40./(NUM/2);
 else if(i>NUM/2)E0=E=40.+(Double_t)(i-(NUM/2))*60./(NUM/2);
 se[3][i]=E0; 
 key=2;
    while(1)   //while循环即模拟粒子入射，每一个循环行进dx个步长，当能量小于零或从最后一个板射出时退出循环，开始下一次模拟
    {
    if((fabs(x-d[0])<=(dx/2)||(x>=0&&x<d[0]))&&E<=0){se[0][i]=E0;se[1][i]=0;se[2][i]=0;key=2;break;} //如果在第一块板中任何地方能量小于零，则第一块板沉积能量就是总能量，并退出本次模拟
    else if(fabs(x-d[0])<=(dx/2)&&E>0){se[0][i]=E0-E;E1=E;key=2;}    //出射第一块板时，能量为初始能量减当前能量，并记录此时能量（亦即入射第二块硅的能量）
    else if((fabs(x-d[1])<=(dx/2)||(x>d[0]&&x<d[1]))&&E<=0){se[1][i]=E1;break;}  //如果在第二块板中...
    else if(fabs(x-d[1])<=(dx/2)&&E>0){se[1][i]=E1-E;E2=E;key=2;  E1=E2=0;break;}    //出射第二块板时...
...
int getit(TGraph *f,double se[4][NUM],int AA,int ZZ,int *I,int _thick)
{
  ...
  TString tele=TString ::Format("/home/zhuhy/zhuhy/pid/pid_tar_ads/pid_300_500/ad3_%dp.root",_thick);
