...
Double_t Ex_0=0,Ex_6600=6.5;
  ...
  for(Theta=20;Theta<50;Theta=Theta+0.05){
    for(Ex=0;Ex<15;Ex=Ex+0.1){
    Q=1.01-((15.0106/14.003241988-1.)*(_15C*15)+(2.01410177811/14.003241988+1.)*Ex-2.*cos(Theta*3.14159/180.)*sqrt(15.0106*2.01410177811*_15C*15*Ex)/14.003241988);
    	if((Q-Ex_6600)<0.01&&(Q-Ex_6600)>-0.01){
...
for(num_ad3=stripnum;;num_ad3=7){
...
//cout<<" Y1"<<'\n';
 ...
 //    cout<<"Num="<<i<<'\t'<<"E="<<E0<<'\t'<<"dE_Si="<<sep[num_ad3][1][i]<<'\t'<<"dE_Mylar="<<sep[num_ad3][3][i]<<'\t'<<"dE_CsI="<<sep[num_ad3][4][i]<<'\n';
  ...
  for(Int_t i=0;i<16;i++){
        nEtheta_d[i]=new TGraph();
  }

  Int_t keypid=1;
  Int_t ip=0,i03=0,i23=0,i43=0;
  for(Int_t j0=stripnum;;j0=7){
      cout<<"******************j="<<j0<<"*****************"<<'\n';
          for(Int_t i0=0;i0<countE0;){
            if(sep[j0][2][i0]>0.1)nEtheta_d[j0]->SetPoint(ip++,thetaE0[i0],sep[j0][2][i0]);        
            i0++;
      }
      ip=0;
      i03=0;
  ...
  nEtheta_d[7]->SetMarkerColor(2);
  nEtheta_d[7]->SetMarkerStyle(6);
  nEtheta_d[7]->Draw("samep");

  nEtheta_d[0]->SetMarkerColor(4);
  nEtheta_d[0]->SetMarkerStyle(6);
  nEtheta_d[0]->Draw("samep");
