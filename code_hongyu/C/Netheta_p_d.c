...
Double_t E_0[10000],E_6600[10000],thetaE0[10000],thetaE6600[10000];
Int_t countE0=0,countE6600=0;
Double_t Ex_0=0,Ex_6600=6.094;
  for(Theta=20;Theta<50;Theta=Theta+0.05){
    for(Ex=0;Ex<15;Ex=Ex+0.01){
    Q=-0.26-((16.0061/15.00011-1.)*(_15C*16)+(2.01410177811/15.00011+1.)*Ex-2.*cos(Theta*3.14159/180.)*sqrt(16.0061*2.01410177811*_15C*16*Ex)/15.00011);
	if(fabs(Q-Ex_0)<0.01){
...
//E0为能量总值，E为能量实时记录值，E1为穿透第一块硅时的能量，E2为穿透第二块时的能量
  ...
  TString  str_1H[4];
  str_1H[0]=TString::Format("/home/zhuhy/JNBook/15C_Etheta/TAT0_datacheck/PID/txt/2H-CH2.txt");
  str_1H[1]=TString::Format("/home/zhuhy/JNBook/15C_Etheta/TAT0_datacheck/PID/txt/2H-Al.txt");
  str_1H[2]=TString::Format("/home/zhuhy/JNBook/15C_Etheta/TAT0_datacheck/PID/txt/2H-Si.txt");
  str_1H[2]=TString::Format("/home/zhuhy/JNBook/15C_Etheta/TAT0_datacheck/PID/txt/2H-CsI.txt");
...
//    getline(in,ss);
    ...
    while(!in_1H.eof())
    {
      ...
      g_1H[0]->SetPoint(i++, e*2,dedx);//添加数据点到图中，能量单位为MeV/u，对C需乘以12.    
    }
  }
  in_1H.close();
...
for(num_ad3=stripnum;;num_ad3=4){
 // cout<<"num_ad3="<<num_ad3<<'\n';
 //cout<<ad3_thick[num_ad3]<<" : "<<ad4_thick[num_ad4]<<'\n';

 d[0]=47.5*ad3_thick[num_ad3]/realthick; 
...
//cout<<" Y1"<<'\n';
 ...
 //    cout<<"Num="<<i<<'\t'<<"E="<<E0<<'\t'<<"dE_Si="<<sep[num_ad3][1][i]<<'\t'<<"dE_Mylar="<<sep[num_ad3][3][i]<<'\t'<<"dE_CsI="<<sep[num_ad3][4][i]<<'\n';
  ...
  for(Int_t j0=stripnum;;j0=4){
      cout<<"******************j="<<j0<<"*****************"<<'\n';
          for(Int_t i0=0;i0<countE0;){
            if(sep[j0][2][i0]>0.1)Etheta_d[j0]->SetPoint(ip++,thetaE0[i0],sep[j0][2][i0]);        
            i0++;
      }
      ip=0;
      i03=0;
  ...
  Etheta_d[4]->SetMarkerColor(4);
  Etheta_d[4]->SetMarkerStyle(6);
  Etheta_d[4]->Draw("samep");
