...
Double_t Ex_0=0.74,Ex_6600=6.094;
  for(Theta=20;Theta<50;Theta=Theta+0.05){
    for(Ex=0;Ex<25;Ex=Ex+0.01){
    Q=-5.98-((16.0061/15.0106-1.)*(_15C*16)+(3.01603/15.0106+1.)*Ex-2.*cos(Theta*3.14159/180.)*sqrt(16.0061*3.01603*_15C*16*Ex)/15.0106);
...
for(num_ad3=stripnum;;num_ad3=8){
...
//cout<<" Y1"<<'\n';
 ...
 //    cout<<"Num="<<i<<'\t'<<"E="<<E0<<'\t'<<"dE_Si="<<sep[num_ad3][1][i]<<'\t'<<"dE_Mylar="<<sep[num_ad3][3][i]<<'\t'<<"dE_CsI="<<sep[num_ad3][4][i]<<'\n';
  ...
  Etheta_d[8]->SetMarkerColor(4);
  Etheta_d[8]->SetMarkerStyle(6);
  Etheta_d[8]->Draw("samep");
