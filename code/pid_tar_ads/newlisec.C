#define NUM 2000 //入射粒子数,按师兄建议一般取十万，得到能量乘积图时为了图案清晰取一万
void openandreadandtest(double se[4][NUM],int M,char str[3][20],int _thick); //模拟粒子入射，每块板上的能量沉积结果存在二维数组se中
...
I=&J;
char str3He[3][20]={"3He-CD2.txt","3He-Al.txt","3He-Si.txt"};
for(Int_t _thick=800;_thick<=800;_thick++){
if(_thick%10==0)cout<<"thick="<<_thick<<'\n';
AA=3;ZZ=2; openandreadandtest(se,AA,str3He,_thick); getit(f,se,AA,ZZ,I,_thick); empty(se);
}
...
void openandreadandtest(double se[4][NUM],int M,char str[3][20],int _thick){
TGraph *g[3];
ifstream in(str[0]);
string ss;
double a, b, e, dedx;
...
TGraph *g[1];
ifstream in1(str[1]);
g[1]=new TGraph;//定义新的二维散点图(TGraph) g1
if(in1.is_open()){//检查文件是否存在
    ...
    while(!in1.eof())
    {
      ...
      // 对照数据文件进行理解。一列一列地读入，提取第3、4列数据。
      g[1]->SetPoint(i++, e*M,dedx);//添加数据点到图中，能量单位为MeV/u，对C需乘以12.    
        }
 } 
 in1.close();
...
TGraph *g[2];
ifstream in2(str[2]);
g[2]=new TGraph;//定义新的二维散点图(TGraph) g1
if(in2.is_open()){//检查文件是否存在
    getline(in2,ss);
    int i=0;
    while(!in2.eof())
    {
      ...
      // 对照数据文件进行理解。一列一列地读入，提取第3、4列数据。
      g[2]->SetPoint(i++, e*M,dedx);//添加数据点到图中，能量单位为MeV/u，对C需乘以12.    
        }
 } 
 in2.close();

 Int_t key=0;
 double E0=0,E1=0,Esmall=0,E2=0,E=0,dE=0,e1=0,e2=0,e3=0,x=0,dx=0.1;//E0为能量总值，E为能量实时记录值，E1为穿透第一块硅时的能量，E2为穿透第二块时的能量
 Double_t d[3]={0.35,0.5,150.};
 d[0]=d[0]*(Double_t)(_thick)/150.;
 d[1]=d[1]*(Double_t)(_thick)/150.;
 d[2]=(Double_t)(_thick);
 ...
 if(i<=NUM/2)E0=E=(Double_t)(i)*400./(NUM/2);
 else if(i>NUM/2)E0=E=400.+(Double_t)(i-(NUM/2))*400./(NUM/2);
 se[3][i]=E0; 
 key=0;
    while(1)   //while循环即模拟粒子入射，每一个循环行进dx个步长，当能量小于零或从最后一个板射出时退出循环，开始下一次模拟
    {
    if((fabs(x-d[0])<=(dx/2)||(x>=0&&x<d[0]))&&E<=0){se[0][i]=E0;se[1][i]=0;se[2][i]=0;key=1;break;} //如果在第一块板中任何地方能量小于零，则第一块板沉积能量就是总能量，并退出本次模拟
    else if(fabs(x-d[0])<=(dx/2)&&E>0){if(key!=1){se[0][i]=E0-E;E1=E;key=1;}}    //出射第一块板时，能量为初始能量减当前能量，并记录此时能量（亦即入射第二块硅的能量）
    else if((fabs(x-d[1])<=(dx/2)||(x>d[0]&&x<d[1]))&&E<=0){se[1][i]=E1;break;}  //如果在第二块板中...
    else if(fabs(x-d[1])<=(dx/2)&&E>0){if(key!=2){se[1][i]=E1-E;E2=E;key=2;}}    //出射第二块板时...
 ...
 //if(i%1==0)cout<<"ESi="<<se[2][i]<<'\t'<<"Eall="<<se[3][i]<<'\n';
 } 
}
...
int getit(TGraph *f,double se[4][NUM],int AA,int ZZ,int *I,int _thick)
{
  ...
  TString tele=TString ::Format("/home/zhuhy/zhuhy/pid/pid_tar_ads/pid_CD2/ad3_%d3He.root",_thick);
